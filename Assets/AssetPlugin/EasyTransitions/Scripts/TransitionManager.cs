...
namespace EasyTransition
{
    ...
    {        
        ...
        public void CustomTransition(string sceneName, TransitionSettings transition)
        {
            if (transition == null || runningTransition)
            {
                Debug.LogError("You have to assing a transition.");
            ...
            runningTransition = true;
            StartCoroutine(CustomTimer(sceneName, transition));
        }
        ...
        IEnumerator CustomTimer(string sceneName,  TransitionSettings transitionSettings)
        {
            //yield return new WaitForSecondsRealtime(startDelay);
        ...
        private IEnumerator Start()
        {
            while (this.gameObject.activeInHierarchy)
            {
                ...
                var managerCount = Object.FindObjectsByType<TransitionManager>(FindObjectsSortMode.None).Length;
                if (managerCount > 1)
